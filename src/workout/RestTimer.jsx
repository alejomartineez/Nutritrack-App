    ...
    >
      <div
        className={`w-full max-w-md rounded-2xl border px-4 py-3 flex items-center justify-between gap-3 pointer-events-auto shadow-lg transition-colors ${
          isDone ? 'bg-entreno-500 border-entreno-400 animate-pulse' : 'bg-neutral-900/95 border-entreno-500/40'
        }`}
      >
        <div className="flex items-center gap-2">
          <Timer className={`w-5 h-5 ${isDone ? 'text-neutral-900' : 'text-entreno-400'}`} />
          <span className={`font-mono text-xl font-black ${isDone ? 'text-neutral-900' : 'text-entreno-300'}`}>
            {mm}:{ss}
          </span>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`p-2 rounded-full ${isDone ? 'hover:bg-entreno-400' : 'hover:bg-neutral-800'}`}
          >
        ...
        <div className="w-full max-w-md flex gap-1.5 justify-center mt-1.5 pointer-events-auto">
          {REST_PRESETS_SEC.map((preset) => (
            <button
              ...
              className="px-2.5 py-1 rounded-full surface text-entreno-300 text-[11px] font-semibold"
            >
              {preset}s
            </button>
          ))}
